---
title: "Reservoir Water Clarity: Station-Level Trend Analysis"
...
```{r}
# Load the data
data <- openxlsx::read.xlsx("data/VADEQ_reservoir_secchi_data_original.xlsx", sheet = 2)
...
# --------- Remove site years that do not have a minimum of 3 measurements in May-Oct ------------
  ...
  ## How many unique station-years have at least 3 measurements for each variable?
  x <- stationyears %>%
...
After initital wrangling, summarizing, and overall assessment of available data, Paul found another DEQ dataset that contains alternative metrics for estimating suspended solids (TF & TSF). This is valuable because of the dirth of available TSS data in the original data set.

Solids data set has more or less the same sort of structure as the original data set. After wrangling it must be incorporated into the main data set.
...
Data must be further subset to include only lake stations that have a minimum of 15 observations. 
The result will be a separate subset of lake stations for each variable.

This subsetting should maybe be done on the individual, variable-specific data frames. Using the joined df could *potentially* introduce station-years with <3 Nobs.
...
# Not the case...joining the dfs did not create a situation in which a variable had only 1 or 2 Nobs per station-year.
...
write_csv(metadata, "data/station_metadata.csv")
            
```

# Create Data Subsets by Variable

Subset data by variable where Nobs >= 15 for that variable

```{r}
subset_variable_data <- function(variable_name, metadata_column, threshold = 15) {
  stations <- metadata %>%
...
subset_data <- list(
  SECCHI_DEPTH = subset_variable_data("secchi", "nYears_Secchi"),   
  CHLa = subset_variable_data("chla", "nYears_CHLA"),           
  NITROGEN_TOTAL = subset_variable_data("nitrogen", "nYears_totNitrogen"), 
  PHOSPHORUS_TOTAL = subset_variable_data("phosphorus", "nYears_totPhosphorus"), 
  TSF = subset_variable_data("TSF", "nYears_TSF"), 
  TF = subset_variable_data("TF", "nYears_TF"),
  TSS = subset_variable_data("TSS", "nYears_TSS"),
  AMMONIA_TOTAL = subset_variable_data("ammonia", "nYears_AMMONIA"),
  NITRATE_TOTAL = subset_variable_data("nitrate", "nYears_NITRATE"),
  DIN = subset_variable_data("DIN", "nYears_DIN")
)
...
"The good news is that we have a high degree of coherence for monitoring of Secchi, TN, TP and CHLa as all have similar start years, mean years and number of stations to be included in the station-specific and population-level trend analysis.

But it is disappointing to see that there are so few stations that have adequate TSS or turbidity data.  This limits us to asking the question whether Secchi is changing over time, and if so, is this due to changes in CHLa.  But of the 3 parameters that most influence water clarity (CDOM, TSS and CHLa), we are missing the two most important.

Please go ahead and run the station-specific analysis for Secchi, TN, TP and CHLa.  Note that for all 4 of these we are working only with the summer (May-Oct) average values for each year."
    ...
    for (month_num in 5:10) {
      ...
      if (nrow(month_station) > 0) {  # Ensure data exists
        ...
        tryCatch({
          ...
          message(paste("Error for station:", station_id, "month:", month_num, ":", e$message))
        })
      }
    }
  }
...
# Loop through each variable and run the function. This generates a heavily nested list structure trends_results -> list for each variable -> each variable has lists for each station_month -> each station_month list contains 1 list with 4 elements (the model results)
for (variable in variables) {
...
write_csv(regressions_df, "data/Regression_Statistics.csv")
