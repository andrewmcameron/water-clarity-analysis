...
trends <- read_csv("data/longTerm_Annual_Regression_Statistics_ALLSTATIONS.csv")
...
for (var in vars) {
  # Get the yearly averages by station for the current variable
  df <- subset_data[[var]]
...
plot_df$significant <- factor(plot_df$significant, levels = c("significant positive", "significant negative", "not significant"))
...
# Plot 4: Total Nitrogen
p4 <- ggplot(subset(plot_df, variable == "NITROGEN_TOTAL")) +
  geom_segment(aes(x = minYear, xend = maxYear, 
                   y = adjusted_intercept, 
                   yend = adjusted_intercept + model_slope * (maxYear - minYear), 
                   color = significant), size = .24) +
...
# Plot 5: Total Phosphorus
#! ---- This plot controls the X-AXIS LABEL (YEAR) ----!#
p5 <- ggplot(subset(plot_df, variable == "PHOSPHORUS_TOTAL")) +
  geom_segment(aes(x = minYear, xend = maxYear, 
                   y = adjusted_intercept, 
                   yend = adjusted_intercept + model_slope * (maxYear - minYear), 
                   color = significant), size = .24) +
...
ggsave("publication_plot.jpg", width = 7, height = 7, units = "in", dpi = 1000)
