...
monthlyAvgs_complete <- c(stationAvgs_10yrs, secchiOnly_avgs)
...
trends_func <- function(station, variable) {
  ...
  df <- monthlyAvgs_complete[[station]][[variable]]
...
for (station in unique(all33_stations)) {
  for (variable in vars) {
    if (variable %in% names(monthlyAvgs_complete[[station]])) {
...
for (station in unique(all33_stations)) {
  ...
  modelSums <- trends_results[[station]]
      ...
      reg.df <- data.frame(STATION_ID = station,
                           variable = vars[vars %in% names(monthlyAvgs_complete[[station]])],
               model_slope = rep(NA, length(monthlyAvgs_complete[[station]])),
               model_MAD = rep(NA, length(monthlyAvgs_complete[[station]])),
               model_pval = rep(NA, length(monthlyAvgs_complete[[station]])),
               model_intercept = rep(NA, length(monthlyAvgs_complete[[station]])))
      
  for (i in seq_along(monthlyAvgs_complete[[station]])) {
...
regressions_df <- bind_rows(regression_statistics) # 28 stations with >= 10 yrs data
...
for (station in unique(all33_stations)) {
  ...
  station_vars <- vars[vars %in% names(monthlyAvgs_complete[[station]])]
  ...
  for (i in seq_along(station_vars)) {
    var <- station_vars[i]
    minYear.df$minYear[i] <- min(monthlyAvgs_complete[[station]][[var]]$Year, na.rm = TRUE)
  }
...
for (station in names(monthlyAvgs_complete)) {
  # Initiate list to store plots for this station
  station_plots <- list()
  
  for (current_var in names(monthlyAvgs_complete[[station]])) {
    df_avgs <- monthlyAvgs_complete[[station]][[current_var]]
    ...
    var_plot <- ggplot(df_avgs, aes(x = Year, y = MAY_OCT_AVG)) +
      geom_point() +
        # Use annotate() instead of aes() inside geom_segment for the trend line. ggplot expects each aesthetic  to be mapped to a column in the data, and geom_segment() - which I originally used - was receiving values (x, xend, y, and yend) that had only a single length, while the data (df_avgs) has multiple rows.  Using `annotate` eliminates the warning because it explicitly adds a line based on single values (rather than having ggplot2 try to match them to rows in df_avgs)
